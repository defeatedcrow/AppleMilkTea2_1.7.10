...
public class FluidDispenser {
	...
	{
		final IBehaviorDispenseItem lastHandler =
				(IBehaviorDispenseItem) BlockDispenser.dispenseBehaviorRegistry.getObject(Items.bucket); 
		...
		final IBehaviorDispenseItem myHandler = new BehaviorDefaultDispenseItem() {
			...
			@Override protected ItemStack dispenseStack(IBlockSource block, ItemStack itemstack)
	        {
		...
		final IBehaviorDispenseItem combinedHandler = new IBehaviorDispenseItem() {
			@Override public ItemStack dispense(IBlockSource block, ItemStack stack) {
	            ...
	            if (world.getBlock(i, j, k) == DCsAppleMilk.blockVegitableOil || world.getBlock(i, j, k) == DCsAppleMilk.blockCamelliaOil)
	            {
	            	isMyItem = true;
	            }
		...
		Map<Item, IBehaviorDispenseItem> internalMap = ObfuscationReflectionHelper.getPrivateValue(
			RegistrySimple.class, (RegistrySimple)BlockDispenser.dispenseBehaviorRegistry, "registryObjects", "field_82596_a");
