...
public class WorldGenYuzuTrees extends WorldGenAbstractTree{
    ...
    public boolean generate(World world, Random rand, int x, int y, int z)
    {
        ...
        if (y >= 1 && y + height + 1 <= 256)
        {
            ...
            else
            {
                ...
                boolean isSoil = block2.canSustainPlant(world, x, y - 1, z, ForgeDirection.UP, (BlockYuzuSapling)DCsAppleMilk.saplingYuzu);
                if (isSoil && y < 256 - height - 1)
                {
                    ...
                    {
                        i3 = k1 - (y + height);
                        l1 = height > 5 ? (i3 < -3 ? b1 - i3 / 2 : b1 + 2 - i3 / 2) : (i3 < -3 ? b1 -1 - i3 / 2 : b1 + 1 - i3 / 2);
                        ...
                        for (i2 = x - l1; i2 <= x + l1; ++i2)
                        {
                            ...
                            for (int k2 = z - l1; k2 <= z + l1; ++k2)
                            {
                                ...
                                if (Math.abs(j2) != l1 || Math.abs(l2) != l1 || rand.nextInt(2) != 0 && i3 != 0)
                                {
                                    ...
                                    {
                                        this.setBlockAndNotifyAdequately(world, i2, k1, k2, DCsAppleMilk.leavesYuzu, meta);
                                    }
                                }
                            }
                        }
                    ...
                    for (k1 = 0; k1 < height; ++k1)
                    {
                        ...
                        if (block.isAir(world, x, y + k1, z) || block.isLeaves(world, x, y + k1, z) || block.getMaterial() == Material.plants)
                        {
    ...
    protected boolean isReplaceable(World world, int x, int y, int z)
    {
        ...
        return block.isAir(world, x, y, z) || block.isLeaves(world, x, y, z) || block.isWood(world, x, y, z) || block == DCsAppleMilk.saplingYuzu || func_150523_a(block);
    }
