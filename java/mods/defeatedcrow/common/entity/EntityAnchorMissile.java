...
public class EntityAnchorMissile extends Entity implements IProjectile{
    ...
    public EntityAnchorMissile(World par1World, EntityLivingBase par2EntityLivingBase, EntityLivingBase par3EntityLivingBase, 
    		float speed, float speed2, float initialYaw, float adjustX, float adjustY, float adjustZ)
    {
    	super(par1World);
    	this.renderDistanceWeight = 10.0D;
    ...
    public void onUpdate()
    {
        ...
        if (this.active)
        {
        	...
        	for (int i = 0; i < 4; ++i)
            {
                this.worldObj.spawnParticle("crit", this.posX + dx, this.posY, this.posZ + dz, -this.motionX, -this.motionY + 0.2D, -this.motionZ);
            }
        }
                        ...
                        if (target instanceof EntityLivingBase)
                        {
                            ...
                            if (this.knockbackStrength > 0)
                            {
                                ...
                                if (f3 > 0.0F)
                                {
                                	//Y方向に大きめに打ち上げる
                                    target.addVelocity(
                                    	this.motionX * (double)this.knockbackStrength * 0.2000000238418579D / (double)f3, 
                                        0.3D, 
                                        this.motionZ * (double)this.knockbackStrength * 0.2000000238418579D / (double)f3);
                                }
                            }
