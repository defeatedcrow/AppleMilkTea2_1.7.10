...
public class TileTeppann extends TileEntity {
	...
	private int getCookingTime() {
		...
		if (DCsConfig.teppannRandomCookTime)
			l += this.worldObj.rand.nextInt(100);
		else
			l = 100;
	...
	private void updateTeppanFoods() {
		...
		if (meta == 2)
			this.cookingItem = (new ItemStack(DCsAppleMilk.foodPlate, ItemCount, 0));
		else if (meta == 4)
			this.cookingItem = (new ItemStack(DCsAppleMilk.foodPlate, ItemCount, 1));
		else if (meta == 6)
			this.cookingItem = (new ItemStack(DCsAppleMilk.foodPlate, ItemCount, 2));
		else if (meta == 8)
			this.cookingItem = (new ItemStack(DCsAppleMilk.foodPlate, ItemCount, 3));
	}
