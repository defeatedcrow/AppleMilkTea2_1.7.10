...
public class BlockCPanel extends BlockContainer{
    ...
    private boolean formWindPlayerEffect(World world, EntityPlayer player, String targetName)
    {
		...
		if (target != null)
		{
			...
			if (flag)
			{
				...
				{
					player.addChatMessage(new ChatComponentText("Fail to warp near the registered player : " + target.getDisplayName() + " is not alive in this dimention."));
				}
				else
				{
			...
			{
				player.addChatMessage(new ChatComponentText("Fail to warp near the registered player : " + "Fail to get position of around " + target.getDisplayName() + "."));
			}
		}
		return false;
    }
    ...
    private boolean formMoonEffect(World world, int x, int y, int z, EntityPlayer player)
    {
		...
		for (int i = 1 ; i < 128 ; i++)
		{
			...
			if (ItemPrincessClam.moonCanWarp(world, X, Y + i, Z) && (world.isAirBlock(X, Y + i + 1, Z) || world.getBlock(X, Y + i + 1, Z).getMaterial() == Material.plants || world.getBlock(X, Y + i + 1, Z).getMaterial() == Material.snow)
