...
public class BlockFilledChocoPan extends BlockContainer {
	...
	public boolean onBlockActivated(World par1World, int par2, int par3, int par4, EntityPlayer par5EntityPlayer,
		...
		if (itemstack == null) {
			ItemStack ret = new ItemStack(DCsAppleMilk.emptyPanGaiden, 1, 0);
			...
			if (!par1World.isRemote) {
				...
				if (pan != null && pan.getRemainByte() > 0) {
					...
					EntityItem entityItem = new EntityItem(par1World, (double) ((float) par2 + f),
							(double) ((float) par3 + f1), (double) ((float) par4 + f2), choco);
			...
			if (chocolate != null && chocolate.getItem() != null) {
				...
				if (par1World.isRemote)
					par5EntityPlayer.addChatMessage(new ChatComponentText(StatCollector
							.translateToLocal("dc.panMessage.noChocolateRecipe")));
				return true;
			}
		}
	}
	...
	private void getChocolate(World world, int x, int y, int z, EntityPlayer player, ItemStack itemstack,
			ItemStack chocolate) {
		...
		if (!world.isRemote) {
			EntityItem entity = new EntityItem(world, player.posX, player.posY, player.posZ, chocolate);
