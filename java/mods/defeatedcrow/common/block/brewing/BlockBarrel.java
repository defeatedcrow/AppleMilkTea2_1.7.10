        ...
        if (Util.notEmptyItem(item) && tile.getAged())
        {
        	...
        	if (item.getItem() == Items.bucket && fluid.amount > 1000)//バケツ
        	{
        		ret = FluidContainerRegistry.fillFluidContainer(
						new FluidStack(fluid.getFluid(), 1000), new ItemStack(Items.bucket));
        	...
        	{
        		ret = FluidContainerRegistry.fillFluidContainer(
						new FluidStack(fluid.getFluid(), 200), new ItemStack(DCsAppleMilk.emptyBottle));
        		if (Util.notEmptyItem(ret)) drainAmount = 200;
        	}
        	else
        	{
        		ret = FluidContainerRegistry.fillFluidContainer(
						new FluidStack(fluid.getFluid(), 1000), new ItemStack(item.getItem(), 1, item.getItemDamage()));
        		if (Util.notEmptyItem(ret)) drainAmount = 1000;
        	}
        ...
        else if (Util.notEmptyItem(item) && tile.productTank.isEmpty())
        {
        	if (FluidContainerRegistry.isFilledContainer(item))
        	{
        		...
        		if (input != null)
        		{
        			...
        			{
        				par5EntityPlayer.addChatMessage(new ChatComponentText("This fluid can not brew in this barrel."));
        			}
        		}
        	}
        	return false;
